...
typedef void*          (*Fn_mono_array_new)(MonoDomain*, MonoClass*, uintptr_t);
...
static Fn_mono_array_new                  g_array_new                  = nullptr;
...
bool MONO_TryInit(void)
{
    ...
    g_array_new      = (Fn_mono_array_new)     GetProcAddress(g_hMono, "mono_array_new");
...
// Allocate a managed byte[] of length `len` and copy `data` into it.
// On 64-bit mono-2.0-bdwgc the array element vector starts at offset
// 0x20 (MonoObject header 0x10 + bounds ptr 0x08 + max_length 0x08).
MonoObject* MONO_NewByteArray(const void* data, int len)
{
    if (!g_bReady || !g_array_new || !g_get_byte_class || len < 0) return nullptr;
    MonoDomain* dom = g_get_root_domain();
    if (!dom) return nullptr;
    MonoClass* bc = g_get_byte_class();
    if (!bc) return nullptr;
    void* arr = g_array_new(dom, bc, (uintptr_t)len);
    if (!arr) return nullptr;
    if (data && len > 0) memcpy((char*)arr + 0x20, data, (size_t)len);
...
bool MONO_StringToUtf8(MonoString* s, char* out, size_t outSize)
{
    if (!out || outSize == 0) return false;
    out[0] = 0;
    if (!s || !g_string_to_utf8) return false;
    char* utf8 = g_string_to_utf8(s);
