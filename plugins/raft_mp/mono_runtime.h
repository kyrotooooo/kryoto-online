...
// GC-managed. Returns the array as MonoObject* (assignable to a byte[]
// field). Used to swap a request's Payload mid-flight.
MonoObject* MONO_NewByteArray(const void* data, int len);

// If `s` is a MonoString, copy its raw UTF-8 value into `out`
