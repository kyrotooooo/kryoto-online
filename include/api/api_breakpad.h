...
{
	KRYOTOLOG("[KryotoOnline] UpdateMinidumpSteamID -> %lld\r\n", sid);

	if (sid == 0) return;

	if (g_pfnBreakpadSetSteamID)
		KRYOTOLOG("[KryotoOnline] Caching SteamID %lld (breakpad loaded: yes)\r\n", sid);
	else
		KRYOTOLOG("[KryotoOnline] Caching SteamID %lld (breakpad loaded: no)\r\n", sid);
...
{
	KRYOTOLOG("[KryotoOnline] LoadBreakpadSymbols\r\n");

	if (!g_bUsingBreakpad)
	{
		KryotoColor(FOREGROUND_RED | FOREGROUND_INTENSITY, "[KryotoOnline] LoadBreakpad: UseBreakpadCrashHandler not called\r\n");
		return;
	}
	...
	if (!hTarget)
	{
		...
		if (!hTarget)
		{
			KryotoColor(FOREGROUND_RED | FOREGROUND_INTENSITY, "[KryotoOnline] LoadBreakpad: no module loaded\r\n");
			return;
		}
	}

	KRYOTOLOG("[KryotoOnline] Loading breakpad functions from steamclient\r\n");
	...
	KRYOTOLOG("[KryotoOnline] Initializing breakpad minidump system\r\n");
...
{
	KRYOTOLOG("[KryotoOnline] SteamAPI_SetBreakpadAppID -> %u\r\n", appId);

	if (g_BreakpadAppID != appId)
	{
		KRYOTOLOG("[KryotoOnline] Setting breakpad AppID: %u\r\n", appId);
...
{
	KRYOTOLOG("[KryotoOnline] SteamAPI_SetMiniDumpComment\r\n");
...
{
	KRYOTOLOG("[KryotoOnline] SteamAPI_UseBreakpadCrashHandler\r\n");
...
{
	KRYOTOLOG("[KryotoOnline] SteamAPI_WriteMiniDump\r\n");
